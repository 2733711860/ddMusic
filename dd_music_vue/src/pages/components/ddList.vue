			...
			<div class="list_item" v-for="(item, index) in list">
				...
				<div class="list_song">
					...
					<div class="play_icon">
						...
						<img :src="likeList.filter(one => one.id==item.id).length>0 ? '../../../static/img/likeing.png' : '../../../static/img/love.png'" title="是否喜欢" v-if="uid!=null" @click="likeIt(item, index)" />
...
export default {
  ...
  computed: {
    ...
    listScroll(e) {
      ...
      if (scrollTop + offsetHeight >= scrollHeight - 50) {
		...
		async playThis(music, index) { // 选择播放
			console.log(music)
			if (this.$route.path == '/music_listPage') { // 正在播放列表页面
    		...
    		music.image = image
				if (this.$route.path == '/music_search') { // 播放搜索到的歌曲先获取图片
				}
				this.selectPlay({
	        list: this.list,
	        index
  	...
  	async _getMusicDetail(musicId) { // 获取音乐详情，返回音乐图片
  	...
  	async likeIt(item, index) { // 喜欢与取消喜欢
  		let param = {
				id: item.id,
				name: item.name,
				singer: item.singer,
    ...
    ...mapActions(['selectPlay', 'clearHistory', 'removeHistory', 'setLikeList'])
	}
}
</script>
